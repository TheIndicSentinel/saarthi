...
/**
 * Unset DataStore key: on-device-only. There is no Settings toggle; cloud /
 * standard SpeechRecognizer fallback is not offered in the UI.
 */
 ...
 * `false` from an older install that allowed phone speech is kept.
 */
 ...
 * is blocked.
 *
 * Default **true**. The Settings row is gone; new installs stay on-device.
 * Existing installs that already stored a value keep it.
 */
@Singleton
